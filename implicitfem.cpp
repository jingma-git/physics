...
void mvmul(const SimulationParameters &params, const Object &obj, const Eigen::Vector3d *x, Eigen::Vector3d *y)
{
    ...
    {
        y[i] = obj.particles[i].mass * x[i]; // mass
    }
    ...
    for (std::vector<Element>::const_iterator e = obj.elements.begin(); e != obj.elements.end(); e++)
    {
        for (unsigned int i = 0; i < 4; i++)
        {
            for (unsigned int j = 0; j < 4; j++)
            {
                y[(*e)[i]] += scale * e->Q * e->stiffness[4 * i + j] * e->Q.transpose() * x[(*e)[j]]; // Stiffness and Damping
            }
        }
    }
}
...
void solve(const SimulationParameters &params, const Object &obj, const Eigen::Vector3d *b, Eigen::Vector3d *x)
{
    ...
    mvmul(params, obj, x, q); // left-hand-side: q = A*x

    for (unsigned int i = 0; i < n; i++)
    {
        r[i] = b[i] - q[i]; // residual
        d[i] = r[i];        // direction at kth step: p(k)
    }
    ...
    {
        // at each step solve A * p(k) = r(k)
        mvmul(params, obj, d, q);             // q(k) = A * p(k)
        double alpha = sigma / ddot(n, d, q); // alpha = (r(k) * r(k)) / (p(k) * A * p(k))
        daxpy(n, alpha, d, x);                // x(k+1) = x(k) + alpha * p(k)
        daxpy(n, -alpha, q, r);               // r(k+1) = r(k) - alpha * A * p(k)
        double sigma_old = sigma;
        sigma = ddot(n, r, r);
        if (sigma < tol || alpha < 1e-20)
            break;
        double beta = sigma / sigma_old; // beta(k) = (r(k) * r(k)) / (r(k+1) * r(k+1))
